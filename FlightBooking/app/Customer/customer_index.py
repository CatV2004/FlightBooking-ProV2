...
from app import app, VNP_HASH_SECRET, VNP_TMN_CODE, VNP_RETURN_URL, VNP_URL, VNP_RETURN_URL, mail
...
@customer_bp.route('/api/send_email', methods=["GET", "POST"])
def send_mail():
    if request.method == "POST":
        order_id = session.get("orderId")
        ...
        try:
            html_content = render_template('bill.html', order_info=order_info)
        except Exception as e:
        ...
        msg.html = html_content  # Gắn nội dung HTML vào email
